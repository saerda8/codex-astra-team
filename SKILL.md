...
运行触发条件固定为：主会话是 `gpt-6-astra` 且推理档位为 medium、high、xhigh、max 或 ultra。从 Astra Medium 起进入协作路由；Low 仍可直接完成。普通解释、追问、确认或状态问题直接回答，不触发协作、不调用 `spawn_agent`，也不恢复旧任务；非 Astra 主会话不触发本策略。
...
Astra Medium 及以上的明确执行型任务必须先真正派工，不论任务大小，不能只做角色判断或写一个标签。Astra 仍负责核心决策、总控和关键验收，并至少把独立调查、实现、测试或复核之一交给匹配角色；协作要求不能阻断项目本身继续执行。
...
8. 通过正常流程完成项目及钩子信任审核，重新加载新会话。做真实正向、负向派发、权限和模型路由验证，并核对每个 Astra Medium 以上任务是否优先创建了匹配的非 Astra 子智能体及可见路由回执；Low 任务可直接完成。不得绕过审批或用合成事件替代真实生效测试。
