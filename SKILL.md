...
GPT-6 Astra 或 Sol 在 Medium 及以上的明确执行型任务必须先真正派工，不论任务大小，不能只做角色判断或写一个标签。主会话仍负责决策、总控和关键验收，并至少把独立调查、实现、测试或复核之一交给匹配角色；协作要求不能阻断项目本身继续执行。
...
GPT-6 Astra 或 Sol 主会话不以 MCP 派工代替模型角色派发。固定顺序是主会话 → Sol/Terra/Luna；边界清楚、重视效率的任务按复杂度选择 Luna 6 High、XHigh 或 Max。已启动的模型角色可按任务需要调用 MCP Worker。MCP 与模型角色不是两套同时抢占主会话的同级门禁。
